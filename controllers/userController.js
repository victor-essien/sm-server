...
export const changePassword = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      const message = "Password succesfully reset.";
...
export const getUser = async (req, res, next) => {
  try {
    ...
    const user = await Users.findById(id ?? userId)
      ...
      .populate({
        path: "chatRecipients.participant",
      });
    if (!user) {
...
export const viewFriends = async (req, res) => {
  try {
    ...
    let queryObject = {};
    queryObject.id = { $ne: userId };
    queryObject.friends = { $in: userId };

    let result = Users.find(queryObject).select(
      "firstName lastName profileUrl profession"
    );
